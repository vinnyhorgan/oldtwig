...
local function get_character_width(character) return core.font:text_width(character) end

--[[ Get character Height ]] ----------------------------------------------------------------------------
-- Get the width of the current character height the current font.
local function get_character_height(character) return core.font:text_height(character) end

--[[ Get character Kerning ]] ----------------------------------------------------------------------------
-- Get the width of the current character height the current font.
local function get_character_kerning(character, character2) character = character or ""; character2 = character2 or ""; return love.graphics.getFont():getKerning(character, character2) end
  ...
  for i = 1, self.current_character do
    ...
    if self.table_string[i] then -- If a valid character then:
      if not self.table_string[i]:match("%" .. special_character[1] .. ".") then -- If it's equal to only 1 or 2 character[s] then:
        ...
        local color = { self.current_color[1] * 255, self.current_color[2] * 255, self.current_color[3] * 255, 255 }
        graphics.print(core.font, str.x + self.cursor.x, str.y + self.cursor.y, self.table_string[i], color) -- Print Text
...
function m.new(rendering, table_settings) -- Todo, configuration at runtime.
  ...
  if type(table_settings.keep_space_on_line_break) =="nil" then table_settings.keep_space_on_line_break = true end
  ...
  self.keep_space_on_line_break = true and table_settings.keep_space_on_line_break
  self.modify_character_width_table = table_settings.modify_character_width_table or {}
  self.default_font = table_settings.font
